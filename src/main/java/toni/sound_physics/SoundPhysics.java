...
import toni.sound_physics.config.ReverbParams;
...
import static toni.sound_physics.Loggers.*;
...
public class SoundPhysics {
    ...
    private static float getBlockReflectivity(BlockPos blockPos) {
        ...
        BlockState blockState = SectionCache.cachedWorldBlockGetter.getBlockState(blockPos);
        if (blockState == null)
            return 0.1F;
...
//    private static @Nullable SoundPhysicsClonedChunkSection getSection(BlockPos blockPos)
//    {
//        var pos = SectionPos.of(blockPos).asLong();
//        return sectionCache.getAndMoveToFirst(pos);
//    }

//    private static @Nullable BlockState getBlockState(BlockPos blockPos)
//    {
//        var section = getSection(blockPos);
//        if (section == null)
//            return null;
//
//        return section.getBlockState(blockPos);
//    }
...
//        if (sectionCache == null || sectionCacheLevel != TODO_REPLACE) {
//            sectionCache = new ClonedChunkSectionCache(TODO_REPLACE);
//            sectionCacheLevel = TODO_REPLACE;
//        }
//
//        ChunkRenderContext context = WorldSlice.prepare(TODO_REPLACE, SectionPos.of(mc.gameRenderer.getMainCamera().getPosition()), sectionCache);

//        var sectionPos = SectionPos.of(mc.gameRenderer.getMainCamera().getBlockPosition());
//        var renderSectionManager = ((RenderSectionAccess)SodiumWorldRenderer.instance()).sound_physics$getRenderSectionManager();
//        var sectionCache = ((SectionCacheAccess) renderSectionManager).sound_physics$getSectionCache();
//
//        ClonedChunkSection cacheSection = sectionCache.acquire(sectionPos.x(), sectionPos.y(), sectionPos.z());
...
//            SoundPhysicsClonedChunkSection section = getSection(blockHitPos);
            BlockState blockHit = SectionCache.cachedWorldBlockGetter.getBlockState(blockHitPos);
            float blockOcclusion = blockHit == null ? 0F : SoundPhysicsMod.OCCLUSION_CONFIG.getBlockDefinitionValue(blockHit);
            ...
            if (blockHit != null && !blockHit.isFaceSturdy(SectionCache.cachedWorldBlockGetter, rayHit.getBlockPos(), sideHit)) {
    ...
    public static BlockHitResult raycast(Vec3 start, Vec3 end, @Nullable BlockPos ignore) {
        if (SectionCache.cachedWorldBlockGetter == null) {
            Vec3 dir = end.subtract(start);
            return BlockHitResult.miss(end, Direction.getNearest(dir.x, dir.y, dir.z), new BlockPos((int) end.x, (int) end.y, (int) end.z));
        }

        return SectionCache.cachedWorldBlockGetter.clip(new ClipContext(start, end, ClipContext.Block.COLLIDER, ClipContext.Fluid.SOURCE_ONLY, mc.player));
    }
